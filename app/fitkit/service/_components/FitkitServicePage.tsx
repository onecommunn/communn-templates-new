...
const FitkitServicePage = ({
  ...
  const fetchData = async () => {
    try {
      ...
      setLoading(true);
      const responce = await fetchFitKitServiceBundle(communityId, serviceName);
        ...
        <div className="mx-auto py-8">
          ...
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="grid md:grid-cols-2 gap-8 mb-10">
              <div className={`bg-gray-200 rounded-xl shadow border p-6 space-y-4 h-[400px] ${index%2 == 0 ? "order-0" : "order-1"}`} />
