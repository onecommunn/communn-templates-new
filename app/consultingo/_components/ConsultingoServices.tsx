...
const ConsultingoServices: React.FC = () => {
    ...
    <section className="bg-[#fcf6e8] py-10 md:py-20 px-6 md:px-20 font-lexend">
      <div className="container mx-auto">
          ...
          <CarouselContent>
            {services.map((service, index) => (
              <CarouselItem key={index} className="basis-1/1 md:basis-1/3">
                <div className="bg-white flex-1 h-full rounded-[32px] p-4 pt-6 md:p-8 flex flex-col items-center text-center shadow-sm hover:shadow-md transition-shadow">
                  <h3 className="text-2xl font-fraunces text-[#4F2910] mb-4">
                    {service.title}
                  </h3>
                  <p className="text-[#6b4f3a] text-sm leading-relaxed mb-10 line-clamp-2">
                    {service.description}
                  </p>
                  ...
                  <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden group cursor-pointer mt-auto">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-black/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="w-14 h-14 bg-[#BC4C37] rounded-full flex items-center justify-center text-white transform scale-50 group-hover:scale-100 transition-transform duration-300 ease-out shadow-xl">
                        <MoveRight size={24} />
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex justify-center mt-12">
          <button className="bg-[#3d2314] cursor-pointer text-white px-8 py-4 rounded-full flex items-center gap-3 group/btn hover:bg-[#BC4C37] transition-all">
